...
input_length = tokens.size(1)  # Length of the input prompt


time_p_start = time.time()

# Prefill stage (first iteration)
with torch.no_grad():
    logits = model(x)  # Process all input tokens
    logits = logits[:, -1, :]
    ...
    print("Prefill stage complete, generated first token")


time_p_end = time.time()
print(f"The prefill stage takes {time_p_end-time_p_start:.4f} \n")

time_d_start = time.time()
# Decode stage (subsequent iterations)
